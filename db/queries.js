...
async function getPerfumeByPrice(id,price) {
  ...
  */
  ...
  `;
    ...
    JSON_AGG(JSON_BUILD_OBJECT('pp_id',perfume_price_id,'price',price,'count',count)) as price_id_count
    FROM (
...
GROUP BY perfume_id,perfume_name,brand_name,image_url,description
    ...
    `,
      ...
      console.log("in getAllitems: ", rows.length > 0 ? rows[0] : 'empty items list');
      return { rows, viewedRows: rows.length + viewedRows };
    }
...
`,
    ...
    */
    async function updatePerfume(id, { name, description, created_at, image_url, count, price, categories, brand }) {
      console.log("in updatePerfume: ", id);
      const perfume_id = Number(id);
      let category_list = categories;
      if (!Array.isArray(categories)) {
        category_list = [categories];
      }
      const categoryUpdates = () => category_list.map(el=>
        pool.query(
          `INSERT INTO perfume_category (perfume_id,category_id) VALUES ($1,$2);`,
          [perfume_id, Number(el)]
        )
      );
      const [perfume_price, ...other] = await Promise.all([
        pool.query(`INSERT INTO perfume_price (perfume_id, price) VALUES ($1, $2)
        ON CONFLICT (perfume_id, price) DO UPDATE SET price = $2 RETURNING perfume_price_id;`, [perfume_id, Number(price)]),
          pool.query(`INSERT INTO perfume_brand (perfume_id, brand_id) VALUES ($1, $2) 
        ON CONFLICT (perfume_id) DO UPDATE SET brand_id = $2;`, [perfume_id, Number(brand)]),      
            pool.query("UPDATE perfumes SET description=$2, image_url=$3, created_at=$4, perfume_name=$5 WHERE perfume_id=$1;",
              [perfume_id, description, image_url, !created_at ? new Date() : created_at, name]),
              pool.query("DELETE FROM perfume_category WHERE perfume_id=$1;",[perfume_id]),
            ]);
            await Promise.all([...categoryUpdates()]);
            console.log(perfume_price, perfume_price.rows, perfume_price.rows[0].perfume_price_id);
            const perfume_price_id = Number(perfume_price.rows[0].perfume_price_id);
            if (count) {
              await pool.query(`INSERT INTO inventory (perfume_price_id, count, date_updated) VALUES ($1,$2,$3)
        ON CONFLICT (perfume_price_id) DO UPDATE SET count = $2, date_updated = $3;`, [perfume_price_id, Number(count), new Date()]);
              }
            }
  ...
  SELECT perfume_id FROM tt LEFT JOIN perfume_category USING (perfume_id) WHERE tt.pcount=1 AND category_id=$1);`,
            ...
            async function deleteStock(pp_id) {
              if (pp_id) {
                await pool.query("DELETE FROM perfume_price WHERE perfume_price_id=$1;", [pp_id]);
              } else {
                const message = `The perfume price id cannot be blank or null to delete it: ${pp_id}`;
                throw new Error(message);
              }
            } 
